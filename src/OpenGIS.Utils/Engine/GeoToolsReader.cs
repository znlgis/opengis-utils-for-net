...
namespace OpenGIS.Utils.Engine
{
    ...
    public class GeoToolsReader : ILayerReader
    {
        ...
        private OguLayer ReadShapefile(string path, string? attributeFilter, string? spatialFilterWkt, Dictionary<string, object>? options)
        {
        ...
        private OguLayer ReadGeoJson(string path, string? attributeFilter, string? spatialFilterWkt)
        {
            ...
            {
                Name = Path.GetFileNameWithoutExtension(path),
                GeometryType = Enums.GeometryType.UNKNOWN
            };
            ...
            {
                layer.GeometryType = MapNtsGeometryType(firstGeometry.GeometryType);
            }
            ...
            if (firstFeature?.Attributes != null)
            {
                foreach (var attrName in firstFeature.Attributes.GetNames())
                {
                    ...
                    {
                        Name = attrName,
                        DataType = InferFieldDataType(value)
                    };
                    layer.AddField(field);
                }
            }
            ...
            foreach (var ntsFeature in featureCollection)
            {
                ...
                {
                    Fid = fid++,
                    Wkt = ntsFeature.Geometry.AsText()
                };
        ...
        private Enums.GeometryType GetGeometryType(ShapeGeometryType shapeType)
        {
            return shapeType switch
            {
                ShapeGeometryType.Point or ShapeGeometryType.PointM or ShapeGeometryType.PointZ => Enums.GeometryType.POINT,
                ShapeGeometryType.LineString or ShapeGeometryType.LineStringM or ShapeGeometryType.LineStringZ => Enums.GeometryType.LINESTRING,
                ShapeGeometryType.Polygon or ShapeGeometryType.PolygonM or ShapeGeometryType.PolygonZ => Enums.GeometryType.POLYGON,
                ShapeGeometryType.MultiPoint or ShapeGeometryType.MultiPointM or ShapeGeometryType.MultiPointZ => Enums.GeometryType.MULTIPOINT,
                _ => Enums.GeometryType.UNKNOWN
            };
        }

        private Enums.GeometryType MapNtsGeometryType(string geometryType)
        {
            return geometryType.ToUpperInvariant() switch
            {
                "POINT" => Enums.GeometryType.POINT,
                "LINESTRING" => Enums.GeometryType.LINESTRING,
                "POLYGON" => Enums.GeometryType.POLYGON,
                "MULTIPOINT" => Enums.GeometryType.MULTIPOINT,
                "MULTILINESTRING" => Enums.GeometryType.MULTILINESTRING,
                "MULTIPOLYGON" => Enums.GeometryType.MULTIPOLYGON,
                "GEOMETRYCOLLECTION" => Enums.GeometryType.GEOMETRYCOLLECTION,
                _ => Enums.GeometryType.UNKNOWN
            };
        }
