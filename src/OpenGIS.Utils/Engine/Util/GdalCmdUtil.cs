...
public static class GdalCmdUtil
{
    ...
    public static GdbGroupModel GetGdbDataStructure(string gdbPath)
    {
        ...
        var model = new GdbGroupModel { GdbPath = gdbPath };
        ...
        for (int i = 0; i < dataSource.GetLayerCount(); i++)
        {
            ...
            if (layer != null) model.LayerNames.Add(layer.GetName());
        }
    ...
    public static string ExecuteOgrInfo(string path)
    {
        ...
        if (dataSource == null) return $"ERROR: Failed to open data source: {path}";
